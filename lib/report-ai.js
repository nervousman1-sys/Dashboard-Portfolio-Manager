...
החזר אך ורק אובייקט JSON תקין (ללא טקסט נוסף, ללא markdown, ללא \`\`\`) במבנה המדויק הבא:
{
  "summary": {
    ...
    "declineReasons": "מסקנה מבוססת-נתונים ומדויקת לגבי כל ירידה בתזרים/רווחיות: הצבע על המנגנון המספרי המדויק מתוך quartersTable (למשל קפיצת CapEx, צניחת תזרים תפעולי, שחיקת שולי-גולמי בכך-וכך נק', גידול בחוב). אם אין ירידה — ציין שהרווחיות/התזרים יציבים או צומחים, עם המספרים. בלי השערות גנריות.",
    ...
    "partnerships": "שותפויות אסטרטגיות ספציפיות בשמן (ספקים, לקוחות-ענק, מיזמים משותפים, שותפי טכנולוגיה/הפצה/ייצור). אם אין מהותיות ידועות — ציין בקצרה.",
    "keyCustomers": "שמות הלקוחות/הלקוחות-הגדולים הספציפיים שעושים עסקים עם החברה (למשל 'Apple, NVIDIA, ספקיות ענן'), ריכוזיות לקוחות אם ידועה, והשווקים הגאוגרפיים המרכזיים. שמות אמיתיים, לא תיאורים.",
    "recentDeals": "עסקאות/חוזים/הזמנות גדולות ספציפיות: אילו בוצעו בשנה האחרונה (שם, צד שני, היקף) ואילו עומדות בפתח/צפויות. נקוב בשמות וסכומים כשידועים. רק אם באמת אין מידע ציבורי — ציין זאת בקצרה.",
...
async function generateSwot(data, KEY, MODELS) {
    ...
    // Gemini frequently returns transient 503 (model overloaded) / 429 (rate, not quota) /
    // 5xx — NOT a real quota wall. Retry the SAME model with short backoff before falling
    // through to the next model, so a momentary overload doesn't surface as "AI unavailable".
    outer:
    for (const model of MODELS) {
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            } catch (e) { lastErr = `gemini(${model}) fetch ${e.message}`; await sleep(400 * (attempt + 1)); continue; }
            if (!gr.ok) {
                lastErr = `gemini(${model}) ${gr.status}`;
                // 503/429/500/502/504 → transient → retry this model after backoff. Others → next model.
                if ([429, 500, 502, 503, 504].includes(gr.status) && attempt < 2) { await sleep(500 * (attempt + 1)); continue; }
                continue outer;
            }
            const gj = await gr.json();
            const text = (((gj.candidates || [])[0] || {}).content || {}).parts?.map(p => p.text).join('\n').trim() || '';
            parsed = extractJson(text);
            if (parsed && parsed.swot) break outer;
            lastErr = `unparseable result from ${model}`;
            break; // parse issue isn't fixed by retrying the same model — move to the next
        }
    }
