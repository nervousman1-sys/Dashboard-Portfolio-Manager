...
async function fetchFmpReport(symbol) {
    ...
    const quarters = inc.slice(0, 8).map(i => {
        ...
        return {
            date: i.date,
            filingDate: i.filingDate || i.acceptedDate || null,   // when the quarter was actually FILED/released (≠ period end)
    ...
    return {
        ...
        asOf: inc[0]?.date || null,                                  // latest PERIOD END (fiscal quarter close)
        reportedDate: inc[0]?.filingDate || inc[0]?.acceptedDate || null, // when that quarter was FILED/released
        quarters,
    };
}
