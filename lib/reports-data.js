...
// Yahoo's fundamentals-timeseries silently throttles/truncates when too many `type` params are
// requested in one URL (~22 here) — it returns an empty/partial result, which left EVERY Israeli
// report (Yahoo-only) and US reports under FMP-quota exhaustion with zero quarters. Fetch the types
// in small parallel chunks and concatenate the result blocks; indexYhResults then merges them.
...
async function _fetchYhTimeseries(symbol, prefix) {
    const chunks = _chunk(YH_ALL_TYPES, 8);
    const blocks = await Promise.all(chunks.map(async (types) => {
        try {
            const r = await fetch(yhSeriesUrl(symbol, prefix, types), { headers: UA });
            const j = await r.json();
