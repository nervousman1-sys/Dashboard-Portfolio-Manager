...
// is also the GENTLEST on rate limits). Yahoo intermittently rate-limits/blocks datacenter IPs
// like Vercel's, returning an empty result; retry once with a short backoff, and rotate the host
// (query1 ↔ query2) since the two front-ends throttle independently.
async function _fetchYhTimeseries(symbol, prefix) {
    const hosts = ['query2', 'query1'];
    for (let attempt = 0; attempt < 2; attempt++) {
        try {
            const url = yhSeriesUrl(symbol, prefix).replace('query2', hosts[attempt % hosts.length]);
