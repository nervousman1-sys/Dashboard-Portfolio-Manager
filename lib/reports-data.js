...
async function fetchYahooStats(symbol) {
    ...
    try {
        ...
        const mods = 'summaryDetail,defaultKeyStatistics,financialData,price,calendarEvents';
        ...
        const sd = res.summaryDetail || {}, ks = res.defaultKeyStatistics || {}, fd = res.financialData || {}, pr = res.price || {}, ce = res.calendarEvents || {};
        const raw = (o) => (o && typeof o.raw === 'number' && isFinite(o.raw)) ? o.raw : null;
        const earn = ce.earnings || {};
        const ed = (earn.earningsDate || []).map(d => d && d.fmt).filter(Boolean);
...
async function fetchReport(symbol, market, opts) {
    ...
    if (!fast) {
        ...
        if (rpoR.status === 'fulfilled' && rpoR.value) result.rpo = rpoR.value;
    }
    return result;
}
...
module.exports = { fetchReport, fetchYahooStats, FMP_KEY };
