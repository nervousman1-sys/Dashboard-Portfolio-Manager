...
async function fetchReport(symbol, market, opts) {
    ...
    // Enrich beta from Finnhub when the source didn't provide it (Yahoo path). Skipped
    // on the fast bulk-prefetch path to avoid an extra call per ticker × hundreds.
    if (!fast && (result.beta == null || (typeof result.beta === 'number' && isNaN(result.beta)))) {
        const b = await fetchFinnhubBeta(sym);
        if (b != null) result.beta = b;
    }
    // Recent insider transactions (US, detail view only) — feeds the report summary.
    if (!fast && mkt === 'us' && FMP_KEY) {
        try { result.insiders = await fetchFmpInsiders(sym); } catch (e) { /* non-fatal */ }
    }
    return result;
}
