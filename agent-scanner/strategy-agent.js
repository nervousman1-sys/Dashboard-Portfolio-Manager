...
if (!SUPABASE_URL || !SERVICE_KEY) fail('Missing SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY');
...
async function processStrategy(s, fxIls) {
    ...
    if (!fired) { await supabase.from('automated_strategies').update({ last_checked: new Date().toISOString() }).eq('id', s.id); return; }
    ...
    if (!claimed || !claimed.length) return; // someone else already claimed it
...
async function runOnce() {
    ...
    if (error) { log('fetch strategies failed:', error.message); return; }
    const rows = data || [];
    if (!rows.length) { log('no active strategies'); return; }
    const fxIls = await getFxIls();
    log(`evaluating ${rows.length} active strategies (USD/ILS fx=${fxIls.toFixed(4)})`);
    for (const s of rows) {
        try { await processStrategy(s, fxIls); } catch (e) { log(`strategy #${s.id} error:`, e.message); }
    }
    log('pass complete');
}
