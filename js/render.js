...
// Display-only: returns an LTR inline-block span that prevents RTL digit truncation.
// Uses toLocaleString for full comma-separated display — never truncates the number.
function formatAssetQuantity(qty) {
    if (qty == null || isNaN(qty)) return '<span class="force-qty" title="0">0</span>';
    ...
    return `<span class="force-qty" title="${num}">${formatted}</span>`;
}
