...
// Display-only: returns an LTR block span that prevents RTL digit truncation.
// Uses toLocaleString for full comma-separated display — never truncates the number.
function formatAssetQuantity(qty) {
    if (qty == null || isNaN(qty)) return '<span class="qty-display-fixed" title="0">0</span>';
    ...
    return `<span class="qty-display-fixed" title="${num}">${formatted}</span>`;
}
